...
async def main():
    ...
    headers = {
        ...
        "referer": "https://amurbooking.com/user/login",
    ...
    async with ClientSession() as session:
        async with session.post(
                url=url, headers=headers, data=payload, proxy=proxy, proxy_auth=proxy_auth, timeout=10
        ) as response:
...
async def main2():
    ...
    result = await BrowserProcessing(account_id=1, work_type=WorkTypes.GET_TRUCKS_LIST).run_task()
    print(result)
